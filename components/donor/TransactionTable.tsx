...
export function TransactionTable({ transactions }: TransactionTableProps) {
  ...
  const data = useMemo(() => {
    ...
    return rows;
  }, [sortBy, statusFilter, transactions]);

  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2">
        <select className="h-10 rounded-md border border-gray-300 px-2" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}>
          <option value="all">All statuses</option>
          <option value="pending">Pending</option>
          <option value="released">Released</option>
          <option value="redeemed">Redeemed</option>
        </select>
        <select className="h-10 rounded-md border border-gray-300 px-2" value={sortBy} onChange={(event) => setSortBy(event.target.value as 'date' | 'amount')}>
            ...
            {data.map((row) => (
              <tr key={row.id} className="border-t border-gray-200">
                ...
                <td className="px-3 py-2">
                  <Badge
                    variant={
                      row.status === 'redeemed' ? 'synced' : row.status === 'released' ? 'active' : 'pending'
                    }
                  >
                    {row.status}
                  </Badge>
